...
# Initialize a list to contain the threads
final_thr = []

# Iterate over the lists of input data
for t in data:
    # Reset parameters
    x = 0
    y = 0
    ...
    # Iterate over the values and append steps to a list.
    for l in t:
        if str(l)[0] == "R":
            for i in range(int(str(l)[1:])):
                x += 1
                thr.append([x, y])
                # print(thr)
        elif str(l)[0] == "L":
    ...
    # Append the list of steps to the list of threads
    final_thr.append(thr)

# Make list of tuples from lists of threads
list1 = [tuple(lst) for lst in final_thr[0]]
list2 = [tuple(lst) for lst in final_thr[1]]

# Get the intersections of the lists
intersects = set(list1).intersection(list2)

# Initialize a list for the result
results = []

# Calculate the Manhattan distance for the intersections
for i, l in enumerate(intersects):
    results.append(manhattan(0, 0, l[0], l[1]))

# Print the intersection closest to the starting point
print("Intersection closest to start:", min(results))

# Initialize lists for steps
steps = []
steps1 = []
steps2 = []

# Iterate over the steps and calculate the least steps to an intersection
for i in intersects:
    for step in range(len(list1)):
        if list1[step] == i:
            steps1.append(step + 1)

for i in intersects:
    for step in range(len(list2)):
        if list2[step] == i:
            steps2.append(step + 1)

for i in range(len(steps1)):
    steps.append(steps1[i] + steps2[i])

print("The least amount of steps:", min(steps))
